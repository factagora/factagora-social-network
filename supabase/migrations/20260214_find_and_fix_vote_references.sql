-- Find and list all functions that reference cv.vote
-- Then drop and recreate any problematic functions

-- First, let's check what functions exist
-- Run this query to see all functions with 'vote' in their definition:
-- SELECT proname, prosrc FROM pg_proc WHERE prosrc LIKE '%cv.vote%';

-- Drop ALL functions that might have the old column reference
DROP FUNCTION IF EXISTS calculate_claim_consensus(UUID);
DROP FUNCTION IF EXISTS update_claim_consensus_trigger();
DROP FUNCTION IF EXISTS calculate_evidence_credibility(UUID);

-- Recreate calculate_claim_consensus with correct column name
