-- NUCLEAR OPTION: Drop and recreate ALL claim-related functions and triggers
-- This ensures all references to cv.vote are eliminated
...
-- Function: Calculate claim consensus (using cv.vote_value NOT cv.vote)
...
-- Function: Update consensus (using cv.vote_value NOT cv.vote)
...
BEGIN
  ...
  -- Calculate average credibility score (supports_claim column doesn't exist)
...
COMMENT ON FUNCTION calculate_claim_consensus IS 'FIXED: Uses cv.vote_value (not cv.vote)';
COMMENT ON FUNCTION update_claim_consensus IS 'FIXED: Uses cv.vote_value (not cv.vote)';
